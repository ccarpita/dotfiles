...
_ansible_list_hosts_in_inventory_file() {
    local grep_opts="-o"
    local inventory_file="${1:-./hosts}"
    local cache_file="$(dirname "$inventory_file")/.$(basename "$inventory_file").completions"
    if [ -f "$cache_file" ]; then
      cache_mod=$(stat -f %m "$cache_file")
      host_mod=$(stat -f %m "$inventory_file")
      if (( cache_mod >= host_mod )); then
        cat "$cache_file"
        return
      fi
    fi
...
_ansible_complete_host() {
    ...
    hosts="$(_ansible_list_hosts_in_inventory_file "$inventory_file")"
