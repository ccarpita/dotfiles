...
if exists('perlhelp_splittype')
	let s:splittype = perlhelp_splittype
else
	let s:splittype = 'vsplit'
endif

let s:edittype = "vsplit"
...
:command! -nargs=? PerlModLookup  call <SID>PerlModLookup(<f-args>)
:command! -nargs=? PerlModOpen  call <SID>PerlModOpen(<f-args>)
:command! -nargs=? OpenMod	call	<SID>PerlModOpen(<f-args>)
...
if !hasmapto('<Plug>PerlModOpenNormal')
    nmap <silent> <unique> <Leader>ps <Plug>PerlModOpenNormal
endif
if !hasmapto('<Plug>PerlModOpenVisual')
    vmap <silent> <unique> <Leader>ps <Plug>PerlModOpenVisual
endif
if !hasmapto('<Plug>PerlModOpenEditNormal')
    nmap <silent> <unique> <Leader>pg <Plug>PerlModOpenEditNormal
endif
if !hasmapto('<Plug>PerlModOpenEditVisual')
    vmap <silent> <unique> <Leader>pg <Plug>PerlModOpenEditVisual
endif
...
nmap <silent> <unique> <script> <Plug>PerlModOpenNormal  :call <SID>PerlModOpen(expand("<cWORD>"), '')<CR>
vmap <silent> <unique> <script> <Plug>PerlModOpenVisual  y:call <SID>PerlModOpen('<c-r>"', '')<CR>
nmap <silent> <unique> <script> <Plug>PerlModOpenEditNormal  :call <SID>PerlModOpen(expand("<cWORD>"), 'edit')<CR>
vmap <silent> <unique> <script> <Plug>PerlModOpenEditVisual  y:call <SID>PerlModOpen('<c-r>"', 'edit')<CR>
...
function <SID>PerlModOpen(...)
	if a:0 == 0 
		let l:mod = input('Enter the perl module to open: ')
		let l:etype = 'vsplit'
	else 
		let l:mod = a:1
		let l:etype = 'vsplit'
	...
	if l:etype == 'edit' 
		...
		let l:etype = s:edittype
	endif

	let l:mod = substitute(l:mod, "[^a-zA-Z0-9_:]", "", "g")
	if l:mod == ''
		return
	endif

	let l:modfile = <SID>PerlModLookup(l:mod)
	if l:modfile != ''
		execute  l:etype." ".l:modfile
	else
		echo "No module file found for " . l:mod
	endif
endfunction


function <SID>PerlModLookup(mod)
	let l:cwd = system("pwd")
	let l:cwd = strpart(l:cwd, 0, (strlen(l:cwd)-1)) 
	let l:cmd = "find-perl-mod " . a:mod . " " . l:cwd . " 2>/dev/null"
	let l:ans = system(l:cmd)
	let l:ans = substitute(l:ans, '\n', '', 'g')
"	echo l:ans
	return l:ans
endfunction
...
function <SID>PerlHelpWindow(command, syntax)
	if s:splittype == 'split'
    	let s:vheight = 19
	else 
		let s:vheight = '' 
	endif

	if a:command =~ "^No documentation found"
		echo a:command
		return
	endif
    ...
    if s:vwinnum >= 0
        ...
        execute s:vheight.s:splittype.' __PerlHelp'
