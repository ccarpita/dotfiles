...
" unlet perl_htmlInString
" unlet perl_htmlInHereDoc
...
" HTML syntax highlighting, destined for multiline HEREDOC strings 
" Added By ccarpita
...
syn keyword perlStatementFiledesc	binmode close closedir eof fileno getc lstat print say dprint printf readdir readline readpipe rewinddir select stat tell telldir write nextgroup=perlFiledescStatementNocomma skipwhite
...
syn keyword perlStatementMoose		class has class_has method with extends
syn keyword perlStatementMooseType	Str Num	
...
if exists("perl_htmlInString")
  syn cluster perlInterpDQ  contains=@htmlTop
  syn cluster perlInterpDQ  contains=@xmlTop
endif
...
"
" Numbers
syn match  perlNumber	"[-+]\=\(\<\d[[:digit:]_]*L\=\>\|0[xXb]\x[[:xdigit:]_]*\>\)"
...
syn region perlQQ		matchgroup=perlStringStartEnd start=+\<q{+ end=+}+ contains=@perlInterpSQ
syn region perlQQ		matchgroup=perlStringStartEnd start=+\<q/+ end=+/+ contains=@perlInterpSQ
syn region perlQQ		matchgroup=perlStringStartEnd start=+\<q[qx]\s*#+ end=+#+ contains=@perlInterpDQ
syn region perlQQ		matchgroup=perlStringStartEnd start=+\<q[qx]\s*|+ end=+|+ contains=@perlInterpDQ
syn region perlQQ		matchgroup=perlStringStartEnd start=+\<q[qx]\s*(+ end=+)+ contains=@perlInterpDQ,perlBrackets
syn region perlQQ		matchgroup=perlStringStartEnd start=+\<q[qx]\s*{+ end=+}+ contains=@perlInterpDQ
syn region perlQQ		matchgroup=perlStringStartEnd start=+\<q[qx]\s*/+ end=+/+ contains=@perlInterpDQ
syn region perlQQ		matchgroup=perlStringStartEnd start=+\<qw\s*#+  end=+#+ contains=@perlInterpSQ
syn region perlQQ		matchgroup=perlStringStartEnd start=+\<qw\s*|+  end=+|+ contains=@perlInterpSQ
syn region perlQQ		matchgroup=perlStringStartEnd start=+\<qw\s*(+  end=+)+ contains=@perlInterpSQ,perlBrackets
syn region perlQQ		matchgroup=perlStringStartEnd start=+\<qw\s*{+  end=+}+ contains=@perlInterpSQ
syn region perlQQ		matchgroup=perlStringStartEnd start=+\<qw\s*/+  end=+/+ contains=@perlInterpSQ
...
if version >= 600
  ...
  if exists("perl_fold")
    ...
    if exists("perl_htmlInHereDoc")
	syn region perlHereDoc	matchgroup=perlStringStartEnd start=+<<\z(\w*HTML\w*\)+    end=+^\z1$+ contains=@perlInterpDQ,@htmlTop  keepend fold
	syn region perlHereDoc	matchgroup=perlStringStartEnd start=+<<\z(\w*XML\w*\)+    end=+^\z1$+ contains=@perlInterpDQ,@xmlTop  keepend fold
    endif
    ...
    if exists("perl_htmlInHereDoc")
	syn region perlHereDoc	matchgroup=perlStringStartEnd start=+<<\z(\w*HTML\w*\)+    end=+^\z1$+ contains=@htmlTop,@perlInterpDQ  keepend
    endif
...
if version >= 508 || !exists("did_perl_syn_inits")
  ...
  HiLink perlStatementMoose	perlType
  HiLink perlStatementMooseType	perlType
