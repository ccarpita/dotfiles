...
syn keyword     phpFunctions    array_fill_keys array_change_key_case array_chunk array_combine array_count_values array_diff_assoc array_diff_uassoc array_diff array_fill array_filter array_flip array_intersect_assoc array_intersect array_key_exists array_keys array_map array_merge_recursive array_merge array_multisort array_pad array_pop array_push array_rand array_reduce array_reverse array_search array_shift array_slice array_splice array_sum array_udiff_assoc array_udiff_uassoc array_udiff array_unique array_unshift array_values array_walk array_walk_recursive array arsort asort compact count current each end extract in_array key krsort ksort list natcasesort natsort next pos prev range reset rsort shuffle sizeof sort uasort uksort usort     contained
...
syn keyword     phpFunctions    call_user_method_array call_user_method class_exists get_class_methods get_class_vars get_class get_declared_classes get_object_vars get_parent_class is_a is_subclass_of method_exists property_exists get_called_class contained
...
syn keyword     phpFunctions   json_decode json_encode  contained

syn keyword     phpFunctionConstant     DATE_ISO8601 SORT_NUMERIC SORT_REGULAR
...
" Structure
syn keyword     phpStructure    extends implements instanceof parent self static   contained
...
syn match       phpIdentifierSimply     "${[^}]*}"      contains=phpOperator,phpParent  contained display
...
" Define
syn keyword     phpDefine       new use    contained
...
syn cluster     phpClConst      contains=phpFunctions,phpIdentifier,phpConditional,phpRepeat,phpStatement,phpOperator,phpRelation,phpStringSingle,phpStringDouble,phpNumber,phpFloat,phpKeyword,phpType,phpBoolean,phpStructure,phpMethodsVar,phpConstant,phpCoreConstant,phpException,phpFunctionConstant
...
" match one line constructs here and skip them at folding
  ...
  syn region    phpFoldFunction matchgroup=Storageclass start="^\z(\s*\)\(\$[a-zA-Z0-9_]\+\s*=\s*\)\?\(abstract\s\+\|final\s\+\|private\s\+\|protected\s\+\|public\s\+\|static\s\+\)*function\s\([^};]*$\)\@="rs=e-9 matchgroup=Delimiter end="^\z1}" contains=@phpClFunction,phpFoldHtmlInside,phpFCKeyword contained transparent fold extend
...
if version >= 508 || !exists("did_php_syn_inits")
  ...
  HiLink         phpFunctionConstant    Function
