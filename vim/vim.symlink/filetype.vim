...
" Apache config file
au BufNewFile,BufRead httpd.conf*,srm.conf*,access.conf*,.htaccess,apache.conf*,*.snooth.com,*.apache setf apache
...
au BufNewFile,BufRead *.cc
	\ if exists("cynlib_syntax_for_cc")|setf cynlib|else|setf cpp11|endif
au BufNewFile,BufRead *.cpp
	\ if exists("cynlib_syntax_for_cpp")|setf cynlib|else|setf cpp11|endif
...
" Distinguish between HTML, XHTML, and Smarty
fun! <SID>FTCheck_html()
	let m = 1
	while m < 20 && m < line("$")
		if getline(m) =~ '{/\?\(if\|include\|assign\|foreach\|\*\|\$\)'
			setf smarty
			return
		endif
		let m = m + 1
...
" Perl
if has("fname_case")
  au BufNewFile,BufRead *.pl,*.PL,*.t,*.T		call FTCheck_pl()
else
  au BufNewFile,BufRead *.pl,*.t			call FTCheck_pl()
endif
...
au BufNewFile,BufRead *.sql,*.pgsql,*.tyb,*.typ,*.tyc,*.pkb,*.pks	setf sql
